...
function genconf(conffile) { // read config file and create a variable from this
	...
	var conf = {allow: [], forbidden: [], redirect: [], options: [], fastupload: {active: "NO"}}; // Object variable will contain config
		...
		if (nextword == "forbidden" | nextword == "allow") {
			...
			while (b == 1) {
				...
				if (nextword == "folder") {
					...
					if (typeof(conf.fastupload.folder) != "undefined") {

						throw new Error("Folder is already defined for FastUpload");
					...
					conf.fastupload.folder = nextwordB;

				} else if (nextword == "maxsize") {
					...
					if (typeof(conf.fastupload.folder) != "undefined" & typeof(conf.fastupload.maxsize) != "undefined") {
...
function ProcessPageInclude(page,PHJS,callback) {
	if (!fs.existsSync(page)) {
		...
		if (fs.statSync(page).isDirectory()) {
			...
			var PHJS2 = {code: PHJS.code, util: PHJS.util, args: PHJS.args, content: new Array(), libs: PHJS.libs, cd: getPath(page), fileName: getFileName(page), ipsrc: PHJS.ipsrc, session: PHJS.session, global: PHJS.global,
						log: PHJS.log,  errorLog: function(e) {
							...
							ProcessPageInclude(page,copyDict(PHJS), (content,session,global,code) => {
								if (content != "ERROR") {
									...
									PHJS.content.push({type: "html", chaine: "<font color='red' size='4'>Error 500 in '"+page+"'</font>\n"});
								}
							});
						}};
