...
function ProcessPageInclude(page,PHJS,callback) {
	if (!fs.existsSync(page)) {
		...
		if (fs.statSync(page).isDirectory()) {
			...
			var PHJS2 = {code: PHJS.code, util: PHJS.util, args: PHJS.args, content: new Array(), libs: PHJS.libs, cd: getPath(page), fileName: getFileName(page), ipsrc: PHJS.ipsrc, session: PHJS.session, includeVars: PHJS.includeVars,
