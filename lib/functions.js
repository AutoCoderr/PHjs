...
function genconf(conffile) { // read config file and create a variable from this
	...
	var conf = {allow: [], forbidden: [], redirect: [], options: [], fastupload: {}}; // Object variable will contain config
...
function canUpload(filename,fastupload) {
	var allow = true;
	for (var i=0;i<fastupload.forbidden.length;i++) {
	...
	if (allow == false) {
		for (var i=0;i<fastupload.allow.length;i++) {
...
function isInRep(rep,inside) {
	...
	var insideSplit = inside.split(delimiter);
...
function getPath(target) {
	...
	var targetsplit = target.split(delimiter);
...
function ProcessPageInclude(page,PHJS,callback) {
	...
	if (!fs.existsSync(page)) {
		...
		if (fs.statSync(page).isDirectory()) {
			...
			var PHJS2 = {code: PHJS.code, util: PHJS.util, args: PHJS.args, content: new Array(), libs: PHJS.libs, cd: getPath(page), fileName: getFileName(page), ipsrc: PHJS.ipsrc, session: PHJS.session, includeVars: PHJS.includeVars, global: PHJS.global,
