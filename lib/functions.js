...
function openInstead(target,conf,path) {
	...
	for (let i=0;i<option.options.length;i++) {
		if (option.options[i].type == "openinstead") {
			if (FileInModel(fileName,option.options[i].file)) {
				return option.options[i].val;
			}
		}
	}
	return target
...
function ProcessPageInclude(page,PHJS,cacheFiles,phjsFileToJsFunc,callback) {
	...
	if (!fs.existsSync(page)) {
		...
		if (fs.statSync(page).isDirectory()) {
			...
			let PHJS2 = {code: PHJS.code, util: PHJS.util, args: PHJS.args, content: new Array(), header: PHJS.header, libs: PHJS.libs, cd: getPath(page), fileName: getFileName(page), ipsrc: PHJS.ipsrc, session: PHJS.session, includeVars: PHJS.includeVars, global: PHJS.global,
			...
			try {
				...
				},PHJS2, function (chaine) { PHJS.content.push(chaine) },function (objet) { PHJS.content.push(JSON.stringify(objet)); PHJS.setHeader("Content-Type", "application/json"); }); // TRY EXECUTE FILE, PUT RESULT IN 'CONTENT' letIABLE, AND RETURN IT
