...
function ProcessPageInclude(page,PHJS, PHjs,callback) {
	...
	if (!fs.existsSync(page)) {
		...
		if (fs.statSync(page).isDirectory()) {
			...
			let PHJS2 = new PHjs(PHJS.args,[],PHJS.header,PHJS.uri,PHJS.host,PHJS.libs,getPath(page),getFileName(page),PHJS.ipsrc,PHJS.session,PHJS.includeVars,PHJS.global,PHJS.log);
