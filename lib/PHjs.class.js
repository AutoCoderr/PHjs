...
class PHJSClass {
	...
	constructor(args,content,header,uri,host,referer,libs,cd,fileName,ipsrc,session,varsToPass,global,O) {
		this.code = 200;
		this.util = util;
	...
	setHeader(keyOrObject,valueOrEcrase = false){
		if (typeof(keyOrObject) == "string" && typeof(value) == "string") {
			this.header[keyOrObject] = value;
	...
	include = (page,callback,exportVariables = false) => {
		...
		this.O.ProcessPageInclude(page, PHJSClass, (code, content) => {
	...
	die = (msg = null) => {
		...
		this.callback(this.code,this.content);
		throw null;
	};
}
