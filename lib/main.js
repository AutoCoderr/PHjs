		...
		if (type == "folder"){
			fs.readdir(path + page, function(err, items) {
				...
				for (var i=0;i<folders.length;i++) {
					res.write("<tr><td><img src='/icons/folder.gif'></td><td><a href='" + page + folders[i] + "/'>" + folders[i] + "</a></td></tr>");
				}
				for (var i=0;i<files.length;i++) {
					ext = files[i].split(".")[files[i].split(".").length-1];
					...
					res.write("<td><a href='" + page + files[i] + "'>" + files[i] + "</a></td></tr>");
				}
				res.end("</table>");
			});
			return;
