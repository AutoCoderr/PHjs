...
function ProcessPage(page,args0,method,libs) {
    var args = {GET: {}, POST: {}};
    if (typeof(args0) == "string"){
       args0 = args0.split("&");
       for(var i=0;i<args0.length;i++) {
           args[method][args0[i].split("=")[0]] = args0[i].split("=")[1]
       }
    }
	...
	for (var i=0;i<htl.length;i++) {
		...
		file = (file.substring(0,htl[i].debut) + "\necho(`" + newht + "`); " + file.substring(htl[i].fin+1,file.length));
	...
	try {
	    ...
	    content = "ERROR";
		console.log("Error when process " + page + " : \n");
		console.log(e);
	}
	return content;
}
