...
const remplace = functionsl.remplace,
	...
	phjsFileToJsFunc = functionsl.phjsFileToJsFunc,

	cacheFiles = functionsl.cacheFiles;
...
const StartServer = function (path,proto,port,options,libs,logaccess,logerror,conffile) {
	...
	const serverScript = (req, res) => { // --------------------------> LE SERVEUR WEB <------------------------------------------------------
		...
		pageExist = fs.existsSync(path + page);

		if (pageExist) {
			type = fs.statSync(path + page).isDirectory() ? "folder" : "file";
		}
		...
		if (page !== delimiter+"icons"+delimiter+"folder.gif"
		 ...
		 && page !== delimiter+"jquery.js") {
			...
			if (pageExist) {
				...
				redirected = RedirectedTo(page,conf,"file");
				if (!redirected) {
					redirected = RedirectedTo(page,conf,"folder");
				}
			}
			...
			openInsteadPage = openInstead(page, conf, path);
			if (openInsteadPage != page) {
				page = openInsteadPage;
				pageExist = true;
		...
		if (pageExist) { // IF PATH EXISTS, SET THE TYPE ('FOLDER' OR 'FILE')
		...
		function ProcessPage(page,args,libs,content, header,callback) { // FUNCTION TO PROCESS HTML PAGE FROM PHJS
			...
			let PHJS = new PHjs(args,content,header,url.parse(req.url).pathname,req.headers.host,libs,getPath(page),getFileName(page),ipsrc,req.session,{},globalVars,log);
