...
const StartServer = function (path,proto,port,options,libs,logaccess,logerror,conffile) {
	...
	const serverScript = (req, res) => { // --------------------------> LE SERVEUR WEB <------------------------------------------------------
		...
		if (O.page !== delimiter+"icons"+delimiter+"folder.gif"
		...
		if (O.pageExist) { // IF PATH EXISTS, SET THE TYPE ('FOLDER' OR 'FILE')
			...
			if (O.page == delimiter+"icons"+delimiter+"folder.gif" //IF PATH DOES NOT EXIST BUT IT IS AN ICON GIF IN THIS LIST, SEND IT
				...
				fs.readFile(__dirname + delimiter + ".." + O.page, function(error, content) {
				...
				fs.readFile(__dirname + delimiter + ".."+delimiter+"libclient" + O.page, function(error, content) {
		...
		if (O.type == "folder"){ // IF TYPE IS 'FOLDER', SEARCH AN INDEX OR AN FILE WITH THE NAME OF FOLDER
			...
			if (O.page != delimiter) {
				for (let i=0;i<exts.length;i++) {
					if (fs.existsSync(path + O.page + (O.page.split(delimiter)[O.page.split(delimiter).length-2]) + exts[i]) & (O.page.split(delimiter)[O.page.split(delimiter).length-2]) != "index") {
						O.setPage(O.page + (O.page.split(delimiter)[O.page.split(delimiter).length-2]) + exts[i]);
						O.setType("file");
					}
				}
		...
		if (mime == null) { // IF NO MIME FOUND
			...
			if (O.ext == "phjs") { // IF FILE IS PHJS, START EXECUTING IT WITH LAUNCHPHJS()
				...
				fs.readFile(O.page, 'utf-8', function(error, content) {
			...
			try {
				cacheFiles[page].func.start(function (code) {
					...
					function (chaine) { // echo() function
						PHJS.content.push(chaine !== undefined && chaine !== null ? chaine.toString() : "");
					},
					function (objet) { // echo_json() function
						if (typeof(objet) !== "object") return;
						PHJS.content.push(JSON.stringify(objet));
						PHJS.setHeader("Content-Type", "application/json");
				}); // TRY EXECUTE FILE, PUT RESULT IN 'CONTENT' VARIABLE, AND RETURN IT
