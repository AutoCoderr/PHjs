...
var StartServer = function (path,proto,port,options,libs,logaccess,logerror,conffile) {
	...
	if (conf.fastupload.active == "YES") {
			...
			var finish = require(conf.fastupload.finish);
					...
					if (typeof(data) != "object") {
						...
						if (typeof(data.name) != "string" | typeof(data.size) != "number" | typeof(data.data) != "string") {
							...
							if (socket.handler == 0) {
								...
								} else if (!canUpload(data.name,conf)) {
									socket.emit('repupload', {type: 'refused', raison: 'File forbidden'});
									logupload(remplace(socket.handshake.address,"::ffff:",""),data.name,"File forbidden",logerror);
								} else {
									socket.SrcName = data.name;
									socket.filename = Math.round(Math.random()*Math.pow(10,3));
							...
							if (socket.handler != 0) {
								...
								if(data.size >= conf.fastupload.maxsize) {
									...
									if(socket.downloaded >= data.size) { //If File is Fully Uploaded
										if (data.name != socket.SrcName & !canUpload(data.name,conf)) {
											socket.emit('repupload', {type: 'refused', raison: 'File forbidden'});
										} else {
											fs.write(socket.handler, socket.data, null, 'Binary', (err, Writen) => {
												...
												socket.downloaded = 0;
												socket.data = "";
												finish("/tmp/"+socket.filename,data.name,data.size,data.token);
												socket.filename = "";
											});
										}
