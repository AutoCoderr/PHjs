...
const rfs = require('require-from-string'),
	  ...
	  paramsFunctions = require(__dirname + "/paramsFunctions.js"),

	  PHJSFunctions = require(__dirname + "/PHJSFunctions.js");


const include = PHJSFunctions.include,
	  
	  global_destroy = PHJSFunctions.global_destroy,

	  session_destroy = PHJSFunctions.session_destroy,

	  setCode = PHJSFunctions.setCode,

	  errorLog = PHJSFunctions.errorLog;

PHJSFunctions = undefined:



const remplace = paramsFunctions.remplace,

	  genconf = paramsFunctions.genconf,

	  getargs = paramsFunctions.getargs,

	  isAuthorized = paramsFunctions.isAuthorized,

	  RedirectedTo = paramsFunctions.RedirectedTo,

	  asMIME = paramsFunctions.asMIME,

	  getPath = paramsFunctions.getPath,

	  getFileName = paramsFunctions.getFileName,

	  ProcessPageInclude = paramsFunctions.ProcessPageInclude,

	  copyDict = paramsFunctions.copyDict,

	  logupload = paramsFunctions.logupload,

	  equal = paramsFunctions.equal,

	  toMin = paramsFunctions.toMin,

	  canUpload = paramsFunctions.canUpload,

	  lenDict = paramsFunctions.lenDict,

	  countOccu = paramsFunctions.countOccu;
	...
	const serverScript = (req, res) => { // --------------------------> LE SERVEUR WEB <------------------------------------------------------
		...
		if (type == "folder"){ // IF TYPE IS 'FOLDER', SEARCH AN INDEX OR AN FILE WITH THE NAME OF FOLDER
			...
			if (page != delimiter) {
				...
				if (fs.existsSync(path + page + (page.split(delimiter)[page.split(delimiter).length-2]) + ".html") & (page.split(delimiter)[page.split(delimiter).length-2]) != "index") {

					page = page + (page.split(delimiter)[page.split(delimiter).length-2]) + ".html";

					type = "file";

				} else if (fs.existsSync(path + page + (page.split(delimiter)[page.split(delimiter).length-2]) + ".js") & (page.split(delimiter)[page.split(delimiter).length-2]) != "index") {

					page = page + (page.split(delimiter)[page.split(delimiter).length-2]) + ".js";

					type = "file";

				} else if (fs.existsSync(path + page + (page.split(delimiter)[page.split(delimiter).length-2]) + ".phjs") & (page.split(delimiter)[page.split(delimiter).length-2]) != "index") {

					page = page + (page.split(delimiter)[page.split(delimiter).length-2]) + ".phjs";
		...
		if (mime == null) { // IF NO MIME FOUND
			...
			let ext = page.split(".")[page.split(".").length-1];
		...
		function lauchphjs() { // START PHJS
			...
			ProcessPage(page,args,libs,content, function (code) { // START FUNCTION TO PROCESS HTML PAGE FROM PHJS, AND PUT THE RESULT IN 'CONTENT' VARIABLE
		...
		function ProcessPage(page,args,libs,content,callback) { // FUNCTION TO PROCESS HTML PAGE FROM PHJS
			...
			let PHJS = {code: 200, util: util, args: args, content: content, libs: libs, cd: getPath(page), fileName: getFileName(page), ipsrc: ipsrc, session: req.session, includeVars: {}, global: globalVars,
						 log: log,  
						 errorLog: errorLog,
						 setCode: setCode, 
			...
			try { 
				...
				file.start(function (code) { 

				   callback(code);

				},PHJS, function (chaine) { PHJS.content.push({type: "html", chaine: chaine}) },function (objet) { PHJS.content.push({type: "json", chaine: objet}) }); // TRY EXECUTE FILE, PUT RESULT IN 'CONTENT' VARIABLE, AND RETURN IT
					...
					if (typeof(data) != "object") {
						...
						if (typeof(data.name) != "string" | typeof(data.size) != "number" | typeof(data.data) != "string") {
							...
							if (socket.handler == 0) {
								...
								} else if (!canUpload(data.name,conf.fastupload[data.id])) {
							...
							if (socket.handler != 0) {
								...
								if(data.size >= conf.fastupload[data.id].maxsize) {
									...
									if(socket.downloaded >= data.size) { //If File is Fully Uploaded
										if (data.name != socket.SrcName & !canUpload(data.name,conf.fastupload[data.id])) {
											socket.emit('repupload', {type: 'refused', raison: 'File forbidden'});
											logupload(remplace(socket.handshake.address,"::ffff:",""),data.name,"Refused: file forbidden",logaccess);
											fs.remove("/tmp/"+socket.filename, function(err) {
												if (err) {
													logupload(remplace(socket.handshake.address,"::ffff:",""),data.name,"Delete '/tmp/"+socket.filename+"' failed : \n"+err,logerror);
												}
											});
										} else {
