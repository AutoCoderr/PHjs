...
const StartServer = function (path,proto,port,options,libs,logaccess,logerror,conffile) {
	...
	const serverScript = (req, res) => { // --------------------------> LE SERVEUR WEB <------------------------------------------------------
		...
		/*const phJsIcons =  [delimiter+"icons"+delimiter+"folder.gif",
							delimiter+"icons"+delimiter+"unknown.gif",
							delimiter+"icons"+delimiter+"image.gif",
							delimiter+"icons"+delimiter+"text.gif",
							delimiter+"icons"+delimiter+"binary.gif",
							delimiter+"icons"+delimiter+"compressed.gif"
							];
		let iconUsed = false;
		for (let i=0;i<phJsIcons.length;i++) {
			if (phJsIcons[i] === page) {
				iconUsed = true;
				break;
			}
		...
		if (iconUsed) {
			...
			const phJsFiles = [delimiter+"socket.io.js",
							   delimiter+"fastupload.js",
							   delimiter+"jquery.js"
			];
			let fileFound = false;
			for (let i=0;i<phJsFiles.length;i++) {
				if (phJsFiles[i] === page) {
					fileFound = true;
					break;
				}
