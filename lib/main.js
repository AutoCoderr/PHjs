...
var StartServer = function (path,type,port,certs) {
	...
	http.createServer(function(req, res) { // --------------------------> LE SERVEUR HTTP <------------------------------------------------------
		...
		if (ext == "mp3") {
			...
			if (fs.existsSync(page)) {
				var content = ProcessPage(page,param,req.method);
				res.writeHead(200, {"Content-Type": "text/html"});
...
function ProcessPage(page,param,method) {
	args = { GET: {}, POST: {} };
	...
	for (var i=0;i<htl.length;i++) {
		console.log("htl[i] : \n" + htl[i]);
		newht = remplace(htl[i],'"',"'");
		newht = remplace(newht,"\n","");
		file = file.replace(htl[i],'echo("' + newht + '");');
	}
	//console.log("fin for");
	...
	file = "function start(args,echo,content) { " + file + " return content; } module.exports = { start: start };"
	file = rfs(file);
	var content = new Array();
	content = file.start(args,function (chaine) { content.push(chaine + " ") },content);
	return content;
}
