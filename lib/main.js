...
var StartServer = function (path,proto,port,options,libs,logaccess,logerror,conffile) {
	...
	var serverScript = function(req, res) { // --------------------------> LE SERVEUR WEB <------------------------------------------------------
		...
		function lauchphjs() { // START PHJS
			ProcessPage(page,args,libs, function (content,session) { // START FUNCTION TO PROCESS HTML PAGE FROM PHJS, AND PUT THE RESULT IN 'CONTENT' VARIABLE
		...
		function ProcessPage(page,args,libs,callback) { // FUNCTION TO PROCESS HTML PAGE FROM PHJS
			...
			file = "function start(callback,PHJS,echo,echo_json) { var print = echo; \n" + remplace(file,"--END--","\ncallback(PHJS.content,PHJS.session,PHJS.global);") + " } module.exports = { start: start };" // PUT FILE IN A FUNCTION FOR EXECUTE HIS JAVASCRIPT CODE
			try {
			...
			var PHJS = {args: args, content: new Array(), libs: libs, cd: getPath(page), ipsrc: ipsrc, session: req.session, global: GlobalVars};
			try { 
				file.start(function (content,session,global) { 
				   GlobalVars = global;
