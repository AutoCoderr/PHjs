...
var StartServer = function (path,proto,port,options,libs,logaccess,logerror,conffile) {
	...
	var webserver = function(req, res) { // --------------------------> LE SERVEUR WEB <------------------------------------------------------
		...
		function ProcessPage(page,args,libs) { // FUNCTION TO PROCESS HTML PAGE FROM PHJS
			...
			//file = "function start(args,echo,content,libs,cd,ipsrc) { var print = echo; \n" + file + " \nreturn content; } module.exports = { start: start };" // PUT FILE IN A FUNCTION FOR EXECUTE HIS JAVASCRIPT CODE
			file = "function start(PHJS,echo) { var print = echo; \n" + file + " \nreturn PHJS.content; } module.exports = { start: start };" // PUT FILE IN A FUNCTION FOR EXECUTE HIS JAVASCRIPT CODE
			try {
			...
			var PHJS = {args: args, content: new Array(), libs: libs, cd: getPath(page), ipsrc: ipsrc}
			try { 
				PHJS.content = file.start(PHJS, function (chaine) { PHJS.content.push(chaine + " \n") }); // TRY EXECUTE FILE, PUT RESULT IN 'CONTENT' VARIABLE, AND RETURN IT
				return PHJS.content;
			} catch(e) {
