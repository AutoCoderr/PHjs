...
var StartServer = function (path,proto,port,options,libs,logaccess,logerror,conffile) {
	...
	if (typeof(conffile) != "string") {
		...
		var conf = {allow: [], forbidden: [], redirect: [], options: [], fastupload: {active: "NO", forbidden: [], allow: []}};
	...
	var serverScript = (req, res) => { // --------------------------> LE SERVEUR WEB <------------------------------------------------------
		...
		function ProcessPage(page,args,libs,callback) { // FUNCTION TO PROCESS HTML PAGE FROM PHJS
			...
			var PHJS = {code: 200, util: util, args: args, content: new Array(), libs: libs, cd: getPath(page), fileName: getFileName(page), ipsrc: ipsrc, session: req.session, includeVars: {},
