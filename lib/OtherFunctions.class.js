...
class OtherFunctions {
	...
	static genconf(conffile,path) { // read config file and create a variable from this
		...
		while (i<confstr.length) {
			...
			if (o.equal(nextword,"forbidden") | o.equal(nextword,"allow")) {
				...
				while (b == 1) {
					...
					conf[o.toMin(nextword)].push(nextwordB);
				...
				if (isnextword('option(')) {
					...
					while (b == 1){
						...
						if (isnextword('file')) {
							...
							if (o.equal(nextword,"as") | o.equal(nextword,"is") | o.equal(nextword,"redirect") | o.equal(nextword,"openinstead")) {
								...
								conf.options[last].options[lastO].type = o.toMin(nextword);
					...
					if (isnextword("recursive")) {
						...
						if (o.equal(nextword,"yes") | o.equal(nextword,"no")) {
							...
							conf.options[last].recursive = o.toMin(nextword);
			...
			for (let j=i;j<confstr.length;j++) {
				...
				if (confstr[j] != " " & confstr[j] != "\n" & confstr[j] != "\r" & confstr[j] != "\t" & confstr[j] != "" ) {
					...
					for(let k=0;k<word.length;k++) {
						...
						} else if (o.toMin(confstr[j+k]) != o.toMin(word[k])) {
	...
	static equal(A,B) {
		return (o.toMin(A) == o.toMin(B) ? true : false);
	}
	...
	static toMin(str) {
		...
		for(let i=0;i<alphapetMin.length;i++) {
			str = o.remplace(str,alphapetMaj[i],alphapetMin[i]);
		}
		return str;
	}
	...
	ProcessPageInclude(page,PHJS, PHJSClass,callback) {
		if (page[0] != "/" & page[1]+page[2] != ":\\") {
			page = PHJS.cd + o.delimiter + page;
		}
		if (!fs.existsSync(page)) {
			...
			if (fs.statSync(page).isDirectory()) {
				...
				let PHJS2 = new PHJSClass(PHJS.args,[],PHJS.header,PHJS.uri,PHJS.host,PHJS.libs,o.getPath(page),o.getFileName(page),PHJS.ipsrc,PHJS.session,PHJS.includeVars,PHJS.global,this);

				try {

					o.cacheFiles[page].func.start(function (code,content) {

						callback(code,content);

					},PHJS2, function (chaine) { PHJS2.content.push(chaine !== undefined && chaine !== null ? chaine.toString() : "") },function (objet) { if (typeof(objet) !== "object") return; PHJS2.content.push(JSON.stringify(objet)); PHJS.setHeader("Content-Type", "application/json"); }); // TRY EXECUTE FILE, PUT RESULT IN 'CONTENT' letIABLE, AND RETURN IT
	...
	phjsFileToJsFunc(page) {
		let file = o.cacheFiles[page].file;

		if (o.countOccu(file,"<?phjs") != o.countOccu(file,"?>")) {
			this.log(500,'ERROR',"\nBad '<?phjs ?>' tags in '"+this.page+"'");
			return false;
		}
		...
		for (let i=0;i<file.length;i++) { // LOCATE NON PHJS CODE IN THE FILE
			...
			} else if (i == file.length-1 & !isphjs) {
		...
		file = o.remplace(file,"<?phjs",""); // DROP PHJS TAGS FROM FILE

		file = o.remplace(file,"?>","");

		if (file.replace("--END--","") == file) {
			file += "\n--END--";
		}
		file = o.remplace(file,"--END--","\ncallback(PHJS.code,PHJS.content);");
		...
		try {
			...
			this.log(500,'ERROR',"\nSyntax error when process '" + this.page + "' : \n" + util.format(e));
	...
	log(code,type,msg) {
		...
		fs.appendFileSync(logfile,
			...
			(date.getFullYear() + '/' + (date.getMonth()+1) + '/' + date.getDate() + "  " + date.getHours() + ':' + date.getMinutes() + ':' + date.getSeconds() + ' => ' + o.remplace(this.req.connection.remoteAddress,'::ffff:','') + ' => ' + this.req.method + ' ' + dst + ' CODE ' + code + ' ; ' + this.req.headers['user-agent'] + ' ' + msg + '\n'));
