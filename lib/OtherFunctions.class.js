...
class OtherFunctions {
	...
	phjsFileToJsFunc(page) {
		...
		for (let i=0;i<file.length;i++) { // LOCATE NON PHJS CODE IN THE FILE
			...
			if (file[i]+file[i+1]+file[i+2]+file[i+3]+file[i+4]+file[i+5] === "<?phjs" || file[i]+file[i+1]+file[i+2] === "<?=") {
				if (isphjs) {
		...
		for (let i=0;i<file.length;i++) {
			if (file[i]+file[i+1]+file[i+2] === "<?=") {
				...
				file = file.substring(0,i+3)+"\necho("+file.substring(i+3,j)+");"+file.substring(j,file.length);
			}
		}
	...
	log(code,type,msg) {
		...
		fs.appendFileSync(logfile,
			...
			(date.getFullYear() + '/' + (date.getMonth()+1) + '/' + date.getDate() + "  " + date.getHours() + ':' + date.getMinutes() + ':' + date.getSeconds() + ' => ' + this.ipsrc + ' => ' + this.req.method + ' ' + dst + ' CODE ' + code + ' ; ' + this.req.headers['user-agent'] + ' ' + msg + '\n'));
