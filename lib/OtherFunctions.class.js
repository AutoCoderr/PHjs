...
class OtherFunctions {
	...
	ProcessPageInclude(page,PHJS, PHJSClass,callback, vars = {}) {
		...
		if (!fs.existsSync(page)) {
			...
			if (fs.statSync(page).isDirectory()) {
				...
				if (typeof(o.cacheFiles[page]) == "undefined") {
					...
					if (!this.phjsFileToJsFunc(page,vars)) {
						callback(null,500);
						return;
					}
				} else {
					...
					if (o.cacheFiles[page].file.length != file.length) {
						...
						if (Object.keys(vars).length !== Object.keys(o.cacheFiles[page].vars).length) {
							o.cacheFiles[page].vars = vars;
							if (!this.phjsFileToJsFunc(page, vars)) {
								callback(null, 500);
								return;
							}
						...
						for (let key in vars) {
							if (typeof(o.cacheFiles[page].vars[key]) == "undefined" ||
								o.cacheFiles[page].vars[key] !== vars[key]) {
	...
	phjsFileToJsFunc(page,vars = {}) {
		...
		for (let key in vars) {
			let howDefine = "\nconst "+key+" = ";
			switch (typeof(vars[key])) {
				case "object":
					try {
						howDefine += JSON.stringify(vars[key]) + ";";
					} catch (e) {
						howDefine += "new Error(`"+o.remplace(e.message,"\n","\\n")+"`);";
					}
					break;
				case "string":
					howDefine += "`"+vars[key]+"`;";
