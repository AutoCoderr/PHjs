			...
			if (fs.statSync(page).isDirectory()) {
				...
				try {
					...
					o.cacheFiles[page].func.start(function (code,content) {
						...
						function (chaine) { // echo() function
							PHJS2.content.push(chaine !== undefined && chaine !== null ? chaine.toString() : "");
						},
						function (objet) { // echo_json() function
							if (typeof(objet) !== "object") return;
							PHJS2.content.push(JSON.stringify(objet));
							PHJS.setHeader("Content-Type", "application/json");
					}); // TRY EXECUTE FILE, PUT RESULT IN 'CONTENT' letIABLE, AND RETURN IT
